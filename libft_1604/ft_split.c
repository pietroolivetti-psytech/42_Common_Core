...
/*   Updated: 2025/04/15 15:09:48 by pileonar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	
	is_malloc_creation_incorrect(char **av, size_t array_pos, size_t len)
{
	size_t	i;
...
static int	av_creation_confirmation(char **av, const char *s, char d)
{
	...
	while (s[i])
	{
		...
		if (len_word)
		{
			if (is_malloc_creation_incorrect(av, j, len_word + 1))
				return (0);
